function [HazardCurveData]= Get_HazardCurve_USGSMaps (MainDirectory,HazardFolderPath,Latitude,Longitude,T,Vs30,IMpoints)
...
for i=indexPeriod:indexPeriod+1
    if  Period(i)>=100
        evalc(['S=load(','''','2008.WUS.',num2str(Period(i)),'.',num2str(Vs30),'.mat','''',')']);
    else
        evalc(['S=load(','''','2008.WUS.0',num2str(Period(i)),'.',num2str(Vs30),'.mat','''',')']);
    end
    
    % Get the SA and the AFE values
    for j=1:size(S.Untitled,1)
        if isnan(S.Untitled(j,3))
        else
            indexSAend=j-1;
            break;
        end
    ...
    SArange=S.Untitled(1:indexSAend,1); % Sa values in the hazard files
    AFE = S.Untitled(indexSAend+1:end,:); % all the AFE values at all coordinates
